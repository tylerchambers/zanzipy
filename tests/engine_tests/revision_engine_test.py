...
class TestRevisionAwareAuthorizationEngine:
    def test_direct_check_respects_write_and_delete_revisions(self) -> None:
        ...
        delete = repo.write(
            WriteContext(DEFAULT_TENANT), (TupleMutation.delete(tuple_),)
        )
    ...
    def test_group_membership_recursion_uses_same_revision(self) -> None:
        registry = SchemaRegistry()
        ...
        edge_write = repo.write(
            WriteContext(DEFAULT_TENANT), (TupleMutation.touch(edge),)
        )
    ...
    def test_tuple_to_userset_recursion_uses_same_revision(self) -> None:
        registry = SchemaRegistry()
        registry.register_many(
            [
                NamespaceDef(
