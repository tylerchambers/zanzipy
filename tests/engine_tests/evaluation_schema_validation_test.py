...
class TestEvaluationSchemaValidation:
    def test_schema_invalid_subject_set_tuple_is_ignored_by_read_apis(self) -> None:
        repo = InMemoryRelationRepository()
        _write_raw(
            repo,
    ...
    def test_schema_invalid_wildcard_tuple_is_ignored_by_read_apis(self) -> None:
        repo = InMemoryRelationRepository()
    ...
    def test_schema_unknown_reverse_bucket_tuples_are_ignored_by_read_apis(
        self,
    ) -> None:
        repo = InMemoryRelationRepository()
        _write_raw(
    ...
    def test_schema_narrowing_invalidates_existing_tuples(
        self,
    ...
    def test_tuple_to_userset_schema_narrowing_stops_following_old_parent_tuples(
        self,
    ) -> None:
        repo = InMemoryRelationRepository()
        client_v1 = _client(
            repo, _inherited_schema(SubjectReference(namespace="folder"))
        )
        ...
        client_v2 = _client(
            repo, _inherited_schema(SubjectReference(namespace="project"))
        )
