...
class TestRewriteRuleDispatcher:
    ...
    def test_rewrite_rule_dispatcher_rejects_unknown_subclass(self) -> None:
        with pytest.raises(
            TypeError, match="Unsupported rewrite rule type: UnknownRule"
        ):
            ConcreteRewriteRuleDispatcher()._dispatch_rewrite_rule(
                UnknownRule(),
                direct=_direct,
